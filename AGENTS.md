...
through pytest. Lint: `ruff check .` (config in `ruff.toml`; ruff is not in
`.venv`, install with `pipx install ruff` or `python3 -m pip install --user ruff`).
...
- Use `obs_state.push_shot()` / `obs_state.broadcast()` for server state and
  ...
  `init`, `shot`, `pressure`, `shot_pressure`, `layout_update`, `club`.
- Key HTTP routes to keep stable: `/api/shot`, `/api/layout`, `/api/bag`,
  `/api/club`,
...
- Design tokens live in `theme.py`; `assets/theme.css` is a committed
  GENERATED file from `scripts/gen_theme_css.py`. Change colors in `theme.py`
  then run `python3 scripts/gen_theme_css.py` — do not hand-edit `overlay.html`
  / `config.html` / `theme.css` hex values.
- `nova_vision_lab.py` (offline Nova IR analysis) deliberately needs
  `opencv-python numpy` which are NOT in `requirements.txt`; it's not imported
  by the app. `bag_transfer.py` only moves club specs (My Bag) between
  machines, leaving shot history in `shanktuary_session_history.json` alone.
